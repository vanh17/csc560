...
class FrameDesc {
  ...
  private:
    int    pageNo;     // the page within file, or INVALID_PAGE if
     ...
     int numberofPins;  // The pin count for the page in this frame
    bool dirtybit;
  ...
  public:
    int pin_count() { return(numberofPins); }
    int pin() { return(++numberofPins); }
    int unpin() {
        numberofPins = (numberofPins <= 0) ? 0 : numberofPins - 1;
        return(numberofPins);
    }
};
